...
/// This expression convert the first 8 bytes of the pipeline UUID to a BIGINT,
/// and takes its absolute value. It is uses to determine which compiler server
...
/// - of a different platform version and are either `Pending` or `CompilingRust`
///
